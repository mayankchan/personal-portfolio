#########################################################################
# Convolutional Neural Network - Fruit Classification Dropout
#########################################################################
...
trianing_generator = ImageDataGenerator(rescale = 1./255) # Scaling values between 0 and 1 for faster computation
...
model.add(Conv2D(filters = 32, kernel_size = (3,3), padding = 'same', input_shape = (img_width,img_height,num_channels))) #convolutional layer, convolving over 2D, there are conv1D used for language or audio and conv3D used for video 
model.add(Activation('relu'))
model.add(MaxPooling2D()) # pool size by defaault is set as 2X2, check Ctrl + i for more help

model.add(Conv2D(filters = 32, kernel_size = (3,3), padding = 'same')) #2nd conv layer 
model.add(Activation('relu'))
...
model_filename = 'models/fruits_cnn_v02.h5' # Chnanged model name to v02 for dropout
...
# # our model was trained on a batch of images, which means it is expecting 4 dim array, 1 dim for no. of images, so we need to conver single image array to 4 dim

# image = np.expand_dims(image, axis = 0)

# image.shape

# image = image * (1./255) # normalization scaling
